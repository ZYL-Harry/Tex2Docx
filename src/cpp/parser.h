...
{
public:
	TexParser(const std::string path = "\0", const std::string ref_path="\0", const std::string& figure_dir="\0", const std::string& cls_path="\0") :
		tex_path(path), ref_path(ref_path), figure_dir(figure_dir), scan_p(0), equation_index(0){ get_default_style(cls_path); };	//构造函数的声明
	...
	void get_default_style(const std::string& style_path);
